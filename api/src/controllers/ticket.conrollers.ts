...
const createdTicket = async (req: PayloadRequest, res: Response, next: NextFunction): Promise<void> => {
  ...
  const {
    stateId,
    newTicket: { title, description, priority },
  } = req.body;
...
const updateTicketDetails = async (
  req: PayloadRequest<TicketParamsType, unknown, TicketBodyType>,
  res: Response,
  next: NextFunction,
): Promise<void> => {
  ...
  try {
    const updateTickettTitle = await Ticket.findByIdAndUpdate(
      ticketID,
...
export { getTicket, createdTicket, updateTicketDetails, updateStateTicket, deleteTicket };
